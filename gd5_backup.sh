...
{
    if ! eval tar -jcvf - $backup_file | ssh -p 37501 $backup_server_user@$backup_server_ip "cat > $backup_save_path/gd5/www/www_`date +"%Y%m%d"`.tar.bz2"
    then
...
function backup_db ()
{
    ...
    do
        ...
        then
            mysqldump  -ubackup -p'5/4kg8vU&eXzZfi' -B $i | bzip2 -c | ssh -p 37501 $backup_server_user@$backup_server_ip "cat > $backup_save_path/gd5/database/${i}_`date +"%Y%m%d"`.sql.bz2"
        else
...
{
    if ! ssh -p 37501 $backup_server_user@$backup_server_ip "/var/scripts/chk_backup_integrity.sh gd5"
    then
