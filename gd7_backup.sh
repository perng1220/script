#!/bin/bash
#Description:
#backup /var/www directory and mysql to gd7
...
{
    if ! eval tar -jcvf - $backup_file | ssh -p 54179 $backup_server_user@$backup_server_ip "cat > $backup_save_path/gd7/www/www_`date +"%Y%m%d"`.tar.bz2"
    then
...
function backup_db ()
{
    ...
    do
        ...
        then
            mysqldump  -ubackup -p'BC/p2kU,pd35wui' -B $i | bzip2 -c | ssh -p 54179 $backup_server_user@$backup_server_ip "cat > $backup_save_path/gd7/database/${i}_`date +"%Y%m%d"`.sql.bz2"
        else
...
{
    if ! ssh -p 54179 $backup_server_user@$backup_server_ip "/var/scripts/chk_backup_integrity.sh gd7"
    then
