...
#       sales=`echo $data | cut -d ";" -f 15`
        mysql -uroot -e "use $db_name;insert into $table_name (vat_number, company, person_in_charge, address, company_tel1, company_tel1_ext, company_tel2, company_tel2_ext, fax, website, contact, contact_tel, contact_tel_ext, contact_mobile, mail, sales, platform) values ('$vat_number', '$company', '$person_in_charge', '$address', '$company_tel1', '$company_tel1_ext', '$company_tel2', '$company_tel2_ext', '$fax', '$website', '$contact', '$contact_tel', '$contact_tel_ext', '$contact_mobile', '$mail', '$sales', 'www.taiwantrade.com.tw')" || exit 1
        sed -i '1d' $csv_file
    else
        exit 2
    fi
