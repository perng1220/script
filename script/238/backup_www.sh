...
case $1 in
    "full" )
        ...
        tar -zcf /backup/www/www_`date +"%Y%m%d"`_full.tar.gz $backup_dir
        ;;
    "diff" )
        #find $backup_dir $find_exclude_dir -o -type f -newer /backup/www/www_`date -d "last saturday" +"%Y%m%d"`_full.tar.gz -print0 | tar --null -czvf \
#/backup/www/www_`date +"%Y%m%d"`.tar.gz  -T  -
        find $backup_dir -type f -newer /backup/www/www_`date -d "last saturday" +"%Y%m%d"`_full.tar.gz -print0 | tar --null -zcvf \
/backup/www/www_`date +"%Y%m%d"`.tar.gz  -T  -
        ;;
     * )
