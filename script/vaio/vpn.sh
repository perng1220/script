...
start_vpn () {
    ...
    if grep -q '^[	 ]*status ' $CONFIG_DIR/$NAME.conf ; then
      ...
      STATUSARG="--status /run/openvpn/HMA.status $STATUSREFRESH"
    fi
    ...
    start-stop-daemon --start --quiet --oknodo \
        --pidfile /run/openvpn/HMA.pid \
        --exec $DAEMON -- $OPTARGS --writepid /run/openvpn/HMA.pid \
        $DAEMONARG $STATUSARG --cd $CONFIG_DIR \
        --config $CONFIG_DIR/$NAME.conf --auth-user-pass $CONFIG_DIR/hmauser.pass \
        $script_security < /dev/null || STATUS=1

    [ "$OMIT_SENDSIGS" -ne 1 ] || ln -s /run/openvpn/HMA.pid /run/sendsigs.omit.d/openvpn.HMA.pid
...
stop_vpn () {
  ...
  if [ "$?" -eq 0 ]; then
    rm -f $PIDFILE
    [ "$OMIT_SENDSIGS" -ne 1 ] || rm -f /run/sendsigs.omit.d/openvpn.HMA.pid
    rm -f /run/openvpn/HMA.status 2> /dev/null
    log_end_msg 0
    killall openvpn
  else
    log_failure_msg "  Unable to stop VPN HMA"
    killall openvpn
  fi
}
...
start)
  ...
  if test -z "$2" ; then
    ...
    if test -z "$AUTOSTART" -o "x$AUTOSTART" = "xall" ; then
      ...
      for CONFIG in `cd $CONFIG_DIR; ls hma_twn$(($RANDOM%4+1)).conf 2> /dev/null`; do
        NAME=${CONFIG%%.conf}
        log_daemon_msg "  Autostarting VPN HMA"
        start_vpn
      done
    else
      ...
      for NAME in $AUTOSTART ; do
        if test -e $CONFIG_DIR/$NAME.conf ; then
          ...
          log_failure_msg "  Autostarting VPN HMA: missing $CONFIG_DIR/$NAME.conf file !"
          STATUS=1
        fi
      done
    fi
  #start VPNs from command line
    ...
    while shift ; do
      ...
      NAME=$1
      if test -e $CONFIG_DIR/$NAME.conf ; then
        log_daemon_msg "  Starting VPN HMA"
        start_vpn
      else
        log_failure_msg "  Starting VPN HMA: missing $CONFIG_DIR/$NAME.conf file !"
       STATUS=1
      fi
    done
  fi
...
stop)
  ...
  if test -z "$2" ; then
    for PIDFILE in `ls /run/openvpn/HMA.pid 2> /dev/null`; do
      NAME=`echo $PIDFILE | cut -c14-`
      NAME=${NAME%%.pid}
      log_daemon_msg "  Stopping VPN HMA"
      stop_vpn
    done
...
#    done
    ...
    while shift ; do
      ...
      if test -e $CONFIG_DIR/$NAME.conf ; then
        ...
        log_warning_msg "VPN HMA: missing $CONFIG_DIR/$NAME.conf file !"
        GLOBAL_STATUS=1
      fi
    done
  fi
